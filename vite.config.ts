...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        '/api': {
          target: 'http://localhost:5000', // Matches the backend PORT in .env
          changeOrigin: true,
          secure: false,
        },
      },
...
/**
 * Mounts the Express API under /api in the Vite dev server middleware stack.
 * In dev this runs in-process — no separate server or proxy needed.
 * In production, run `tsx server/index.ts` as a sidecar.
 */
function healthPassportApiPlugin(): Plugin {
  let handler: ((req: IncomingMessage, res: ServerResponse, next: (err?: unknown) => void) => void) | null = null
    ...
    async configureServer(server) {
      try {
        ...
        try { await migrate() } catch (e: any) {
          console.warn('[api] DB not available:', e.message, '— running with mock data fallback')
        }
        // Express 5 app is a request handler
        handler = app as any
      } catch (e: any) {
        console.warn('[api] failed to start API plugin:', e.message)
      }

      // Mount at root so Express receives the full /api/* path
      server.middlewares.use((req, res, next) => {
