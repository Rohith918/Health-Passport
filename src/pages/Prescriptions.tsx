...
export default function Prescriptions() {
  const [docs, setDocs] = useState<ApiPrescription[]>([])
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error("[Prescriptions] load error", err.message)
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const addFiles = async (files: File[]) => {
    ...
    try {
      const uploaded = await Promise.all(
        files.map((f) => prescriptionsApi.upload(f, uploadDate)),
      )
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      console.error("[Prescriptions] delete error", err.message)
    }
    setDeleteConfirm(null)
  }

  const handleView = (id: string) => {
    window.open(prescriptionsApi.downloadUrl(id), "_blank")
  }
      ...
      >
        ...
        <div>
            ...
            >
              {uploading ? (
                ...
                <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
                  <path
                    d="M11 14V3M7 7l4-4 4 4"
                    stroke={dragOver ? "#3D8A5F" : "#9C9181"}
                    strokeWidth="1.6"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M3 16v2a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-2"
                    stroke={dragOver ? "#3D8A5F" : "#9C9181"}
                    strokeWidth="1.6"
                    strokeLinecap="round"
                  />
          ...
          >
            ...
            >
              Documents are encrypted at rest and never shared with third
              parties.
            </span>
          </div>
        </div>
                  ...
                  {deleteConfirm === doc.id ? (
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      ...
                      <button
                        ...
                        onMouseEnter={(e) => {
                          e.currentTarget.style.borderColor = "#F0B9A5"
                          e.currentTarget.style.color = "#D9502E"
                        }}
                        onMouseLeave={(e) => {
                          e.currentTarget.style.borderColor = "#EDE8DC"
                          e.currentTarget.style.color = "#9C9181"
                        }}
                      >
                        <svg
                          width="14"
